...
| mechanic | SYDNEY translation | verdict |
|---|---|---|
| Act 2, the city's work | A pool of 100 quests, ten to a rung 1–10, authored by 200 writers and judged per rung; gated on the story (rung 1 waits for `act0:trained`, the rest for `act1:open`) so a new player sees only the Centrelink chain's markers, a rung holds more than a player can finish (a menu, not a chain), and none are repeatable | adopt; passes rules 1, 4, 6, 7 and 8, and is gated on rule 3 (the week is the epic) — a player lured off rung 1 before the chain is done loses the story until Monday |
| Givers have bodies | A dialog NPC is drawn as one more pedestrian (id-stable kit, street-facing heading, idle, turns to face you inside 2× its radius), not an actor: no collision, not hittable, not on the wire | adopt; "a body you cannot push is a worse invisible wall than no body at all" (`client/src/game/giverbodies.ts`) |
| Police stand down when the investigation ends | The five-star convergence needs a live investigation (`heat.ts`, `verifyStandDown`); air and road answer to the star count, foot police to the investigation | adopt |
| Turf | The 86 streaming hexagons are the regions; a KO of somebody not on your side is a point for your side there, a lead of two takes the hexagon, Monday clears it. Drawn as the big map's opening rung (the whole disc, each hexagon washed in its holder's colour, the edge of the world as a dashed circle) and one feed line per change of hands, named for the nearest suburb. No buffs, no locks, no spawn changes: a scoreboard with a shape | adopt; passes 1 (named for suburbs), 3 (the week), 6 (a wash and one line), 7 (nothing to buy or gain but the map), 8 (604 bytes, only on a change) — see `TERRITORY.md` |
